...
export function ValuesSection() {
  return (
    <section className="py-20 px-3 lg:py-28">
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-16 items-start">
          ...
          <div className="divide-y divide-border">
            {values.map((value) => (
              <div
                key={value.number}
                className="group py-8 grid sm:grid-cols-[80px_1fr] gap-4 hover:bg-muted/30 -mx-4 px-4 rounded-xl transition-colors duration-200"
              >
                <div className="font-mono text-3xl font-bold text-muted/50 group-hover:text-primary/30 transition-colors pt-1">
                  {value.number}
                </div>
                <div>
